...
export class HealthController {
  ...
  async ready(_req: Request, res: Response): Promise<void> {
    ...
    try {
      ...
      // In production, this would verify the TTS model is loaded
      checks.model = true;
    } catch (error) {
  ...
  async health(_req: Request, res: Response): Promise<void> {
    ...
    try {
      dependencies.model = {
        status: 'up',
        name: process.env.TTS_MODEL || 'xtts'
      };
    } catch (error) {
      dependencies.model = {
        status: 'down',
    ...
    res.status(allHealthy ? 200 : 503).json({
      ...
      dependencies,
      metadata: {
        model: process.env.TTS_MODEL || 'xtts',
        max_concurrency: process.env.TTS_MAX_CONCURRENCY || 5,
        max_text_chars: process.env.MAX_TEXT_CHARS || 5000
      }
    });
  }
